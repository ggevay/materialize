...
impl RedundantJoin {
    ...
    pub fn action(
        ...
        // Uncomment the follwoing lines to trace the individutal steps:
...
impl ProvInfoCtx {
    /// Extend the `uses` map by the `LocalId`s used in `expr` and remove
    ...
    pub fn remove_uses(&mut self, expr: &MirRelationExpr) {
        ...
        while let Some(expr) = worklist.pop() {
            ...
            // terms enclosed in `Let`/`LetRec` nodes in order to avoid double
            // counting (those are handled by a nested `remove_uses` call).
            match expr {
                MirRelationExpr::Let { body, .. } => {
                    worklist.push(body); // skip value
                }
                MirRelationExpr::LetRec { body, .. } => {
                    worklist.push(body); // skip values
                }
                _ => {
