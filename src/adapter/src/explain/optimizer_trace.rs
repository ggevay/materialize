...
use crate::explain::insights::{self, FastPathCluster, PlanInsightsContext};
            ...
            ExplainStage::PlanInsights => {
                ...
                if let Some(insights_ctx) = insights_ctx {
                    ...
                    if let (Some(plan_insights), false) = (plan_insights.as_mut(), is_fast_path) {
                        if insights_ctx.fast_path_limit {
                            plan_insights.fast_path_limit =
                                Some(insights_ctx.optimizer_config.features.persist_fast_path_limit);
                        }

                        for (cluster_id, idx_id, on_id) in insights_ctx.fast_path_clusters.iter() {
                            let idx_name = {
                                let idx_entry = insights_ctx.catalog.get_entry_by_global_id(idx_id);
                                soft_assert_or_log!(*on_id == idx_entry.index().expect("must be index").on, "AAAAAAAA"); //////// todo   !! the expect is failing !!
                                Some(FastPathCluster {
                                    index: insights::structured_name(humanizer, *idx_id),
                                    on: insights::structured_name(
                                        humanizer,
                                        idx_entry.index().expect("must be index").on,
                                    ),
                                })
                            };
                            let cluster_name = insights_ctx.catalog.get_cluster(*cluster_id).name.clone();
                            plan_insights.fast_path_clusters.insert(cluster_name, idx_name);
                        }
                    }
                }
