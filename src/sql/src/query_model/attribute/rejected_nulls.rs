        ...
        mz_ore::stack::maybe_grow(|| {
            if let Some(c) = case_not_isnull(expr) {
                ...
                if sign {
                    ...
                    intersect_variadic(exprs.iter().map(|e| {propagated_nulls(e)}).collect())
                }
            } else if let Some(exprs) = case_or(expr) {
                if sign {
                    intersect_variadic(exprs.iter().map(|e| {rejected_nulls(e, sign)}).collect())
                } else {
                    union_variadic(exprs.iter().map(|e| {propagated_nulls(e)}).collect())
                }
            } else {
                propagated_nulls(expr)
            }
...
fn union_variadic<T>(mut sets: Vec<HashSet<T>>) -> HashSet<T> //ggtodo: same as in intersect_variadic
    where
        T: Clone + Eq + std::hash::Hash,
{
    sets.iter_mut().fold(HashSet::new(), |s1, s2| {union(s1, s2.clone())})
}
...
fn intersect_variadic<T>(mut sets: Vec<HashSet<T>>) -> HashSet<T> //ggtodo: consume iterator instead?. Also, vec<&...>? Also, avoid the clone
    where
        T: Clone + Eq + std::hash::Hash,
{
    sets.iter_mut().fold(HashSet::new(), |s1, s2| {intersect(s1, s2.clone())})
}
