        ...
        maybe_grow(|| {
            ...
            Ok(match self {
                ...
                TopK {
                    ...
                    let offset = offset.try_into_literal_int64().expect("Should be a Literal by this time").try_into().expect("Should have checked non-negativity of OFFSET clause already");
    ...
    pub fn lower_uncorrelated(self) -> Result<MirScalarExpr, PlanError> {
        ...
        Ok(match self {
            ...
            Select { .. } | Exists { .. } | Parameter(..) | Column(..) | Windowing(..) => {
                sql_bail!("Internal error: unexpected HirScalarExpr in lower_uncorrelated: {:?}", self);
            }
        })
    }
}
