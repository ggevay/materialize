...
fn plan_query_inner(qcx: &mut QueryContext, q: &Query<Aug>) -> Result<PlannedQuery, PlanError> {
    ...
    let offset = match &q.offset {
        None => HirScalarExpr::literal(Datum::Int64(0), ScalarType::Int64),
        Some(offset) => {
            let ecx = &ExprContext {
                qcx,
                name: "OFFSET",
            ...
            let offset = offset.cast_to(ecx, CastContext::Explicit, &ScalarType::Int64)?; ////// todo: Int64 ok?

            let offset = if offset.is_constant() {
                // Simplify it to a literal or error out. (E.g., the cast inserted above may fail.)
                let offset = offset.try_into_literal_int64()?;
                if offset < 0 {
                    sql_bail!("OFFSET must not be negative");
                }
                HirScalarExpr::literal(Datum::Int64(offset), ScalarType::Int64)
            } else {
...
pub fn plan_nested_query(
    ...
    if limit.is_some() || !offset.clone().try_into_literal_int64().is_ok_and(|offset| offset == 0) {
        ...
        match s.distinct {
            ...
            Some(Distinct::On(exprs)) => {
                ...
                relation_expr = HirRelationExpr::top_k(
                    ...
                    HirScalarExpr::literal(Datum::Int64(0), ScalarType::Int64),
    ...
    F4: Fn(ScalarType) -> HirScalarExpr,
{
    ...
    if planned_query.limit.is_some() || !planned_query.offset.clone().try_into_literal_int64().is_ok_and(|offset| offset == 0) {
...
fn plan_map_subquery(
    ...
    if query.limit.is_some() || !query.offset.clone().try_into_literal_int64().is_ok_and(|offset| offset == 0) {
