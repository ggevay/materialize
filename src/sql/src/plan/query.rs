...
/// Plans a non-windowed aggregate function.
fn plan_aggregate(ecx: &ExprContext, func: &Function<Aug>) -> Result<AggregateExpr, PlanError> {
    // todo: if nothing else comes here, then fuse this with `..._common`, and move this check into
    //       the caller.
    if func.over.is_some() {
        unreachable!("Window aggregate; AggregateTableFuncVisitor explicitly filters these out");
    }
    plan_aggregate_common(ecx, func)
}
...
fn plan_function<'a>(
    ...
    if over.is_some() {
        unreachable!("All supported window functions should have been handled by the above match");
    }
