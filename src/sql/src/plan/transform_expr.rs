...
use crate::plan::expr::{
    AbstractExpr, AggregateFunc, HirRelationExpr, HirScalarExpr,
};
    ...
    fn extract_conjuncted_subqueries(expr: &mut HirScalarExpr, out: &mut Vec<HirScalarExpr>) {
        match expr {
            HirScalarExpr::CallVariadic {
                ...
                exprs.into_iter().for_each(|e| extract_conjuncted_subqueries(e, out));
            }
            ...
            HirScalarExpr::Select(input) => {
                ...
                match func {
                    ...
                    AggregateFunc::All => {
                        ...
                        let expr = expr.take();
                        let filter = expr.clone()
                            .not()
                            .or(expr.call_is_null());
                        *e = input
                            .take()
                            .filter(vec![filter])
                            .exists()
