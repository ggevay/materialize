...
use crate::plan::expr::{AbstractExpr, AggregateFunc, AggregateWindowExpr, ColumnRef, HirRelationExpr, HirScalarExpr, ValueWindowExpr, ValueWindowFunc, WindowExpr};
...
pub fn fuse_window_functions(
    ...
    if !context.config.enable_value_window_function_fusion { ////// todo: && !other_flag
        return Ok(());
    }
                ...
                WindowFuncCallOptions::Agg(options) => {
                    let (fused_funcs, fused_args): (Vec<_>, Vec<_>) = self
                        ...
                        .map(|(_idx, call)| {
                            ...
                            {
                                (func.clone(), (**expr).clone()) ////// todo: ** kell? (a fontinel igen)
                            } else {
                                panic!("unknown window function in FusionGroup")
                            }
                        })
    ...
    let is_value_or_agg_window_func_call = |scalar_expr: &HirScalarExpr| -> bool {
        ...
        match scalar_expr {
            HirScalarExpr::Windowing(WindowExpr {
                ...
                !matches!(func, ValueWindowFunc::Fused(..)) && context.config.enable_value_window_function_fusion
            }
            HirScalarExpr::Windowing(WindowExpr {
                func: WindowExprType::Aggregate(AggregateWindowExpr {
                    aggregate_expr: AggregateExpr {
                ...
                !matches!(func, AggregateFunc::FusedWindowAgg { .. }) //////// todo: && other_flag
            }
            _ => false,
        }
    };
            ...
            HirRelationExpr::Map { input, scalars } => {
                ...
                let value_or_agg_window_func_calls = scalars
                    ...
                    .filter(|(_idx, scalar_expr)| {
                        is_value_or_agg_window_func_call(scalar_expr)
                    })
