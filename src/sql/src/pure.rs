...
use mz_sql_parser::ast::{
    ...
    CsrSeedProtobufSchema, DbzMode, DeferredItemName, DocOnIdentifier, DocOnSchema, Envelope, Expr,
    Function, FunctionArgs, Ident, KafkaConfigOption, KafkaConfigOptionName, KafkaConnection,
    KafkaSourceConnection, MaterializedViewOption, MaterializedViewOptionName, PgConfigOption,
...
/// Purifies a CREATE MATERIALIZED VIEW statement.
pub fn purify_create_materialized_view_options(
    catalog: impl SessionCatalog,
    mz_now: Option<Timestamp>,
    options: &mut Vec<MaterializedViewOption<Aug>>,
    ...
    for option in options.iter_mut() {
        ...
        if let Some(WithOptionValue::Refresh(RefreshOptionValue::Every(
            RefreshEveryOptionValue { starting_at, .. },
        ))) = &mut option.value
        {
            if starting_at.is_none() {
                *starting_at = Some(mz_now_expr.clone());
            }
        }
