...
use mz_sql_parser::ast::{
    ...
    DocOnIdentifier, DocOnSchema, Envelope, Expr, Function, FunctionArgs, Ident, KafkaConfigOption,
    KafkaConfigOptionName, KafkaConnection, KafkaSourceConnection, MaterializedViewOption,
    ...
    let (mz_timestamp_id, mz_timestamp_type) = {
        let item = catalog
            .resolve_type(&PartialItemName {
                database: None,
                // This is currently in `pg_catalog`, but will be moved to `mz_catalog` (#23833).
                // For now, `None` should be ok too: For this to not work, a user would have had to
                // have altered their search path to have placed something ahead of `pg_catalog` and
                // had a type named `mz_timestamp`, which is impossible because we prevent users
                // from creating objects with the `mz_` prefix.
                schema: None, // TODO: change to `mz_catalog` when the above is fixed.
