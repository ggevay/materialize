    ...
    I: IntoIterator<Item = Datum<'a>>,
{
    ...
    let mut sort_by = |left: &(_, Row), right: &(_, Row)| {
        let left_order_by = &left.1;
        let right_order_by = &right.1;
        let left_datums = left_datum_vec.borrow_with(left_order_by);
        let right_datums = right_datum_vec.borrow_with(right_order_by);
