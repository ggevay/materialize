...
class MaterializedViewReplacementDropInput(Check):
    ...
    manipulate phase. The zero-downtime scenarios run that phase while the next
    generation is already booted read-only, so the next boot must not pair the
    new definition with the expressions it cached before the apply. The tables
    retain history so that a restart cannot hit CPU-95 (an input's read
    frontier overtaking the output's write frontier).
    """
